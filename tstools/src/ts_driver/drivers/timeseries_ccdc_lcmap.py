...
class LCMAPCCDCTimeSeries(timeseries_stacked.StackedTimeSeries):
    ...
    def __init__(self, location, config=None):
        if not has_scipy:
            raise ImportError('Cannot import "scipy" module required to read '
                              'CCDC results files')
        super(LCMAPCCDCTimeSeries, self).__init__(location, config=config)

    def fetch_results(self):
        """ Read results for current pixel
        """
        # TODO: LCMAP application now has `self.fetch_georef`
        #       -> [tuple[mx (float), my (float), crs_wkt (str)]
        mx, my, crs_wkt = self.fetch_georef

        # TODO: This should come from a LCMAP API call and should be a list of
        #       dict
        result = []

        if not result:
            logger.error('Could not find result for y/x %d/%d' % (my, mx))
            return
        self.ccdc_results = result
